...
function convertRPM(byteA, byteB) {
	return Math.round(((parseInt(byteA, 16) * 256) + parseInt(byteB, 16)) / 4);
}
function convertSpeed(byte) {
	return Math.round(parseInt(byte, 16));
}
...
function convertThrottlePos(byte) {
	return Math.round(parseInt(byte, 16) * (100 / 255));
}
...
function convertSupportedPIDS(byteA, byteB, byteC, byteD) {
	// BE 3E A8 11
	//console.log("convertSupportedPIDS:", byteA, byteB, byteC, byteD);

	var allBits = "";

	for (var i = 0; i < byteA.length; i++) {
		var bits = parseInt(byteA[i], 16).toString(2);
		allBits+= bits = "0000".substr(bits.length)+bits;
		//console.log(byteA[i], ":", bits);
	}

	for (var i = 0; i < byteB.length; i++) {
		var bits = parseInt(byteB[i], 16).toString(2);
		allBits+= bits = "0000".substr(bits.length)+bits;
		//console.log(byteB[i], ":", bits);
	}

	for (var i = 0; i < byteC.length; i++) {
		var bits = parseInt(byteC[i], 16).toString(2);
		allBits+= bits = "0000".substr(bits.length)+bits;
		//console.log(byteC[i], ":", bits);
	}

	for (var i = 0; i < byteD.length; i++) {
		var bits = parseInt(byteD[i], 16).toString(2);
		allBits+= bits = "0000".substr(bits.length)+bits;
		//console.log(byteD[i], ":", bits);
	}
	...
	for(var i = 0; i < allBits.length; i++) {
		...
		if(bit == "1") {
			var pid = i + 1;
			pid = pid.toString(16).length == 1 ? "0" + pid.toString(16).toUpperCase() : pid.toString(16).toUpperCase();
...
var responsePIDS = [
	//Realtime data
	{mode: modeRealTime, pid: "00", bytes: 4, name: "pidsupp0",     description: "PIDs supported 00-20", min: 0, max: 0, unit: "Bit Encoded", convertToUseful: convertSupportedPIDS},
	...
	{mode: modeRealTime, pid: "20", bytes: 4, name: "pidsupp2",     description: "PIDs supported 21-40", min: 0, max: 0, unit: "Bit Encoded", convertToUseful: convertSupportedPIDS},
	...
	{mode: modeRealTime, pid: "40", bytes: 4, name: "pidsupp4",     description: "PIDs supported 41-60", min: 0, max: 0, unit: "Bit Encoded", convertToUseful: convertSupportedPIDS},
